...
public class MoongateTcpServerTests
{
    ...
    {
        using var server = new MoongateTCPServer(new IPEndPoint(IPAddress.Loopback, 0));
        server.AddMiddleware(new UppercaseMiddleware());

        var connectedTcs = new TaskCompletionSource<MoongateTCPClientEventArgs>(TaskCreationOptions.RunContinuationsAsynchronously);
        var dataTcs = new TaskCompletionSource<MoongateTCPDataReceivedEventArgs>(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    public async Task StartAsync_WhenServerClosesClient_ShouldRaiseDisconnectEvent()
    {
        ...
        var connectedTcs = new TaskCompletionSource<MoongateTCPClientEventArgs>(TaskCreationOptions.RunContinuationsAsynchronously);
        var disconnectedTcs = new TaskCompletionSource<MoongateTCPClientEventArgs>(TaskCreationOptions.RunContinuationsAsynchronously);
        server.OnClientConnect += (_, args) => connectedTcs.TrySetResult(args);
    ...
    public async Task StartAsync_WhenMiddlewareThrows_ShouldRaiseOnException()
    {
        ...
        var exceptionTcs = new TaskCompletionSource<MoongateTCPExceptionEventArgs>(TaskCreationOptions.RunContinuationsAsynchronously);
